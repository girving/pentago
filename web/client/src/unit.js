...
import { midsolve, midsolve_tiled, instantiate } from './mid_sync.js'
...
// cheap tests run on every shard.  Without bazel (plain node unit.js) there
// is one shard and everything runs.
const shards = +process.env.TEST_TOTAL_SHARDS || 1
const shard = +process.env.TEST_SHARD_INDEX || 0
if (process.env.TEST_SHARD_STATUS_FILE)  // Tell bazel we support sharding
  writeFileSync(process.env.TEST_SHARD_STATUS_FILE, '')
let ticket = 0
const mine = () => ticket++ % shards == shard

// The set of board names a midsolve of b produces: b and its moves, plus
// grandchild moves when b is not a middle board, stopping below finished
// boards (midsolve solves down to the first non-middle descendants).
// Validated against actual results in test_path, and relied on by the
// analytic completeness check there.
function produced(board) {
...
// The tiled solver must agree with midsolve, single threaded and with helper workers, in both modes
async function test_tiled() {
  for (const [threads, merged] of [[1, false], [4, false], [4, true]]) {
    if (!mine())
      continue
    const start = Date.now()
    const results = await midsolve_tiled(BigInt(mid_board), threads, merged)
    assert.equal(Object.keys(results).length, 1+18+8*18)
    for (const k of Object.keys(results)) {
      const board = parse_board(k)
      assert(board.name in mid_correct)
      assert.equal(mid_correct[board.name], results[k])
    }
    console.log('    tiled time (' + threads + ' threads' + (merged ? ', merged' : '') + ') = ' + (Date.now()-start) / 1000 + ' s')
  }
}
...
async function test_path() {
  ...
  await Promise.all(all_games().map(async ({path, values}) => {
    const deep = path.map(parse_board).filter(b => b.count >= 18)
    await Promise.all(deep.map(async board => {
      if (!mine())
        return
      const results = await compute(board.raw + '')
      // The produced() closure must match the actual result set exactly:
      // the completeness check below depends on it
      const names = new Set(Object.keys(results).map(r => parse_board(r).name))
      assert.deepStrictEqual(names, produced(board))
    ...
    // Analytic completeness: every deep values entry must be produced by
    // some solve above (cheap, so it runs on every shard)
    const covered = new Set()
    for (const board of deep)
      for (const name of produced(board))
        covered.add(name)
    for (const v in values) {
      const board = parse_board(v)
      if (board.count - board.middle >= 18 && !covered.has(board.name))
        throw Error('missed board ' + v + ', count = ' + board.count)
    }
  }))
...
async function toplevel() {
  let tests = [test_moves, test_done, test_rotator_paths, test_wasm, test_mid, test_tiled, test_path]
