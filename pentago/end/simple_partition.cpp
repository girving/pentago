...
GEODE_DEFINE_TYPE(simple_partition_t)
...
void simple_partition_test() {
  ...
  for (int ranks=3<<2;ranks<=(3<<18);ranks<<=2) {
    ...
    // Check totals
    GEODE_ASSERT(sections->total_nodes==total);
    GEODE_ASSERT(sections->total_blocks==total_blocks);
    GEODE_ASSERT(partition->rank_offsets(ranks)==vec(total_blocks,total));
    GEODE_ASSERT(partition->owner_work.sum()==total);
    auto o = partition->other_work.sum();
    if (other==(uint64_t)-1)
      other = o;
    GEODE_ASSERT(other==o);
    const Box<double> excess(1,1.06);
    GEODE_ASSERT(excess.contains(partition->owner_excess));
    GEODE_ASSERT(excess.contains(partition->total_excess));
    ...
    GEODE_ASSERT(max_blocks==(uint64_t)partition->max_rank_blocks);
    GEODE_ASSERT(total_lines==rank_total_lines);
    ...
    if (ranks>=196608) {
      ...
      for (int i=0;i<100;i++) {
        ...
        GEODE_ASSERT((uint64_t)owned.size()==partition->rank_count_lines(rank,true));
      ...
      GEODE_ASSERT(cross || ranks==786432);
    }
  }

  double ratio = (double)other/total;
  cout << "other/total = "<<ratio<<endl;
  GEODE_ASSERT(3.9<ratio && ratio<4);
}
...
void wrap_simple_partition() {
  GEODE_FUNCTION(simple_partition_test)

  typedef simple_partition_t Self;
  Class<Self>("simple_partition_t")
    .GEODE_INIT(int,const sections_t&,bool)
    .GEODE_FIELD(max_rank_blocks)
    .GEODE_FIELD(max_rank_nodes)
    .GEODE_FIELD(owner_starts)
    .GEODE_FIELD(other_starts)
    ;
}
