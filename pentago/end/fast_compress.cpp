...
int fast_compress(RawArray<Vector<super_t,2>> uncompressed, RawArray<uint8_t> compressed, const event_t event) {
  ...
  GEODE_ASSERT((size_t)compressed.size()>=1+snappy::MaxCompressedLength(input_size));
  size_t output_size;
...
void wrap_fast_compress() {
  GEODE_FUNCTION(fast_compress)
  GEODE_FUNCTION(fast_uncompress)
  GEODE_FUNCTION(local_fast_compress_test)
  GEODE_FUNCTION(local_fast_uncompress_test)
}
