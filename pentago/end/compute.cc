...
line_data_t::line_data_t(const line_t& line)
  ...
  GEODE_ASSERT(section_shape.remove_index(dim)==child_section_shape.remove_index(dim));
...
__attribute__((const)) static inline tuple<section_t,uint8_t> standardize_child_section(section_t section, int dimension) {
...
line_details_t::line_details_t(const line_data_t& pre, const wakeup_t& wakeup)
  ...
  // Standardize
  , standard_child_section(get<0>(standardize_child_section(pre.line.section,pre.line.dimension)))
  , section_transform(     get<1>(standardize_child_section(pre.line.section,pre.line.dimension)))
  ...
  , rmin(get<0>(rotation_minimal_quadrants(pre.line.section.counts[0])),
         get<0>(rotation_minimal_quadrants(pre.line.section.counts[1])),
         get<0>(rotation_minimal_quadrants(pre.line.section.counts[2])),
         get<0>(rotation_minimal_quadrants(pre.line.section.counts[3])))
  ...
  , self(this)
{
  ...
  if (section_transform & 4)
    const_cast_(reflection_moves) = vec(get<1>(child_rmin[0]),get<1>(child_rmin[1]),
                                        get<1>(child_rmin[2]),get<1>(child_rmin[3]));
  ...
  if (standard_input_shape != Vector<int,4>()) {
    Array<local_symmetry_t> all_reflection_symmetries(standard_input_shape.sum());
    const_cast_(reflection_symmetry_offsets[0]) = 0;
    int offset = 0;
    for (int i=0;i<4;i++) {
      const auto symmetries = all_reflection_symmetries.slice(offset, offset+standard_input_shape[i]);
      offset += standard_input_shape[i];
      const_cast_(reflection_symmetry_offsets[i+1]) = offset;
      if (section_transform & 4)
        for (int j=0;j<standard_input_shape[i];j++) {
          const quadrant_t q = get<0>(child_rmin[i])[block_size*first_child_block[i]+j];
          symmetries[j].local = (rotation_minimal_quadrants_inverse[pack(reflections[unpack(q,0)],reflections[unpack(q,1)])]&3)<<2*i;
        }
    }
    const_cast_(this->all_reflection_symmetries) = all_reflection_symmetries;
  }
}
...
Vector<uint8_t,4> line_details_t::input_block(int k) const {
  GEODE_ASSERT((unsigned)k<(unsigned)input_blocks);
...
RawArray<Vector<super_t,2>> line_details_t::input_block_data(Vector<uint8_t,4> block) const {
  ...
  GEODE_ASSERT(block.remove_index(child_dim)==first_child_block.remove_index(child_dim));
  return input_block_data(block[child_dim]);
}

RawArray<Vector<super_t,2>> line_details_t::output_block_data(int k) const {
  GEODE_ASSERT((unsigned)k<pre.line.length);
...
static GEODE_UNUSED void slow_verify(const char* prefix, const board_t board, const Vector<super_t,2>& result, bool verbose=false) {
...
void compress_output_block(line_details_t* const line, const int b) {
  ...
  GEODE_ASSERT(char_view(block_data).size()>=4+local_compressed.size());
...
template<bool slice_35> static void compute_microline(line_details_t* const line,
                                                      const Vector<int,3> base) {
  // Prepare
  ...
  GEODE_ASSERT(rmin.size()==pre.output_shape[dim]);
  ...
  for (int i=0;i<rmin.size();i++) {
    ...
    while (moves) {
      ...
      if (slice_35) {
        ...
        GEODE_ASSERT(after_board==transform_board(symmetry,child_board));
...
#endif
        wins |= rmax(transform_super(symmetry,~child[1]));
        not_loss |= rmax(transform_super(symmetry,~child[0]));
      }
    }
    // Finish up
    ...
    result[0] = (wins&~immediate)|(wins0&~wins1);
    result[1] = (not_loss&~immediate)|wins0;
  }
