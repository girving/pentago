...
readable_block_store_t::readable_block_store_t(const shared_ptr<const block_partition_t>& partition,
                                               const int rank, RawArray<const local_block_t> blocks,
  ...
  , store(store, blocks.size())
{
  ...
  for (const auto& block : blocks) {
    ...
    const_cast_(block_infos)[block.local_id] = info;
    const_cast_(block_to_local_id)[make_tuple(info.section,info.block)] = block.local_id;
  }
...
accumulating_block_store_t::accumulating_block_store_t(
    const shared_ptr<const block_partition_t>& partition, const int rank,
  ...
  for (const auto section : sections->sections) {
    ...
    for (int i=0;i<samples_per_section;i++) {
      ...
      if (const auto* local_id = get_pointer(block_to_local_id, make_tuple(section,block)))
  ...
  for (const auto section : sections->sections) {
    ...
    for (int i=0;i<samples_per_section;i++) {
      const auto index = random.uniform(Vector<int,4>(),shape);
      const auto block = Vector<uint8_t,4>(index/block_size);
      if (auto* local_id = get_pointer(block_to_local_id, make_tuple(section,block))) {
...
restart_block_store_t::restart_block_store_t(
    const shared_ptr<const block_partition_t>& partition, const int rank,
    RawArray<const local_block_t> blocks, const shared_ptr<compacting_store_t>& store)
  : Base(partition, rank, blocks, store) {}
...
const block_info_t& readable_block_store_t::block_info(const local_id_t local_id) const {
  if (const auto* info = get_pointer(block_infos, local_id))
    return *info;
...
void readable_block_store_t::print_compression_stats(const reduction_t<double,sum_op>& reduce_sum) const {
  ...
  for (auto& info : block_infos) {
    const int flat_id = get<1>(info).flat_id;
    const int k = 64*block_shape(get<1>(info).section.shape(),get<1>(info).block).product();
    uncompressed_ += k;
  ...
  if (root) {
    ...
    slog("compression ratio = %g +- %g", mean, dev);
  }
#endif
}
...
void accumulating_block_store_t::accumulate(local_id_t local_id, uint8_t dimension, RawArray<Vector<super_t,2>> new_data) {
  ...
  if (!info.missing_dimensions) {
    ...
    const int section_id = check_get(sections->section_id, info.section);
    spin_t spin(section_counts_lock);
    section_counts[section_id] += counts;
  }
