...
#if defined(BUILDING_pentago_core) || defined(BUILDING_pentago_mpi)
#include "gen/tables.h"
#else
#include <pentago/gen/tables.h>
#endif
namespace pentago {
...
GEODE_EXPORT bool black_to_move(board_t board);

// Throw ValueError if a board is invalid
GEODE_EXPORT void check_board(board_t board);

GEODE_EXPORT board_t standardize(board_t board);

// Random board and side generation
GEODE_EXPORT side_t random_side(Random& random);
GEODE_EXPORT board_t random_board(Random& random);

// Generate a random board with n stones
GEODE_EXPORT board_t random_board(Random& random, int n);
...
GEODE_EXPORT string str_board(board_t board);
