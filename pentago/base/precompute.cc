...
string slow_won() {
  ...
  // Start ourselves off
  file += autogenerated;
  line("#include \"pentago/base/board.h\"");
  line("namespace pentago {\n");

  line("bool slow_won(const side_t side) {");
  line("  bool won = false;");
  for (const int q : range(4))
  ...
  for (const auto p : quadrant_set_win_patterns) {
    ...
    Array<uint16_t,2> masks(pats.size(), quads.size());
    for (const int ip : range(pats.size()))
      for (const int iq : range(quads.size()))
        masks(ip, iq) = uint16_t(pats[ip] >> 16*quads[iq]);
    ...
    line("  static const quadrant_t %s%s[%d] = %s;", mask, leading, quads.size(), init);
    line("  %swon |= %s;", loop, joins(" & ", quads.size(),
        [&](int iq) { return format("!(q%d & %s[%d])", quads[iq], ith, iq); }));
    counter++;
  }
  line("\n  return won;");
  line("}\n\n}  // namespace pentago");
