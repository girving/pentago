...
const int for_js = 1, for_wasm = 2, for_py = 4;
...
// Reformat an initializer for Javascript or Python
string script_init(const string& init) {
  string s;
...
unordered_map<string,string> save() {
  ...
  }
  {
    ...
    for (const auto& [type, name, sizes, init, flags] : tables)
      if (flags & for_js)
        js += format("exports.%s = %s\n", name, script_init(init));
    files["tables.js"] = js;
  }
  {
    string py;
    py += "#" + autogenerated.substr(2);
    py += "import numpy as np\n\n";
    for (const auto& [type, name, sizes, init, flags] : tables) {
      if (flags & for_py) {
        py += format("%s = np.asarray(%s, dtype=np.%s)\n", name, script_init(init), dtype(type));
        py += format("%s.flags.writeable = False\n", name);
      }
    }
    files["tables.py"] = py;
  }
...
REMEMBER(commute_global_local_symmetries,
  ...
  remember("uint8_t", "commute_global_local_symmetries", "%d", table, for_py);
)
