...
// type, name, c++ sizes, c++ initializer
vector<tuple<string,string,string,string>> tables;
...
template<class Data> void
remember(const string& type, const string& name, const string& fmt, const Data& data) {
  ...
  tables.emplace_back(type, name, sizes, init.str());
}

// Save tables to .h and .cc files
void save(const string& h, const string& cc) {
  const string note = "// Autogenerated by precompute: do not edit directly\n\n";
  {
    ofstream file(h.c_str());
    file << note;
    file << "#pragma once\n\n#include <cstdint>\n";
    file << "namespace pentago {\n\n";
    for (const auto& [type, name, sizes, init] : tables)
      file << format("extern const %s %s%s;\n", type, name, sizes);
    file << "\n}\n";
  } {
    ofstream file(cc.c_str());
    file << note;
    file << "#include \"pentago/base/gen/tables.h\"\n";
    file << "namespace pentago {\n\n";
    for (const auto& [type, name, sizes, init] : tables)
      file << format("const %s %s%s = %s;\n", type, name, sizes, init);
    file << "\n}\n";
  }
}
...
REMEMBER(moves,
  ...
  for (const uint16_t filled : range(1 << 9)) {
    ...
    GEODE_ASSERT(mv.size() == 9-popcount(filled));
    extend(flat, mv);
...
template<class T> struct View {
  ...
  bool singleton(const int axis) const {
    GEODE_ASSERT(unsigned(axis) < rank());
  ...
  View swapaxes(const int a0, const int a1) const {
    GEODE_ASSERT(unsigned(a0) < rank());
    GEODE_ASSERT(unsigned(a1) < rank());
...
REMEMBER(rotation_minimal_quadrants,
  ...
  remember("uint16_t", "rotation_minimal_quadrants_offsets", "%d", nest.offsets);
  remember("uint16_t", "rotation_minimal_quadrants_flat", "%d", nest.flat);
  remember("uint16_t", "rotation_minimal_quadrants_inverse", "%d", inverse);
...
int main(int argc, char** argv) {
  try {
    GEODE_ASSERT(argc == 3);
    const string h = argv[1];
    const string cc = argv[2];
    const auto endswith = [](string x, string y) {
