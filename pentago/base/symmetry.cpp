...
static void group_test() {
  ...
  for (auto g : symmetries) {
    GEODE_ASSERT(g*e==g);
    GEODE_ASSERT(e*g==g);
    GEODE_ASSERT(g*g.inverse()==e);
  }

  // Test cancellativitiy
  for (auto a : symmetries)
    for (auto b : symmetries) {
      GEODE_ASSERT((b*a.inverse())*a==b);
      GEODE_ASSERT(a*(a.inverse()*b)==b);
    }
  ...
  GEODE_ASSERT(generated.size()==symmetries.size());
  GEODE_ASSERT(generators.size()<=integer_log(symmetries.size())+2);
  ...
  for (auto a : symmetries)
    for (int r : range(256)) {
      ...
      symmetry_t sb(b);
      GEODE_ASSERT(a*b==a*sb);
      GEODE_ASSERT(b*a==sb*a);
    }
}
...
static void action_test(int steps) {
  ...
  for (int step=0;step<steps;step++) {
    ...
    GEODE_ASSERT(transform_side(s0,transform_side(s1,side0))==transform_side(s0*s1,side0));
    GEODE_ASSERT(transform_board(s0,pack(side0,side1))==pack(transform_side(s0,side0),transform_side(s0,side1)));
  }
}
...
static NdArray<board_t> superstandardize_py(NdArray<const board_t> boards) {
  NdArray<board_t> standard(boards.shape,uninit);
...
super_t transform_super(symmetry_t s, super_t C) {
  ...
  #define TRANSPOSE_23() ({ /* 18 ops */ \
    ...
    const auto m = geode::pack(0,-1,0,-1); \
...
static void super_action_test(int steps) {
  ...
  for (int step=0;step<steps;step++) {
    ...
    GEODE_ASSERT(correct==transform_super(s,C));
    check_interrupts();
  }
  ...
  for (int step=0;step<steps;step++) {
    ...
    GEODE_ASSERT(super_meaningless(transform_board(s,board))==transform_super(s,super_meaningless(board)));
    check_interrupts();
  }
}
...
void wrap_symmetry() {
  GEODE_FUNCTION(group_test)
  GEODE_FUNCTION(action_test)
  GEODE_FUNCTION(superstandardize_test)
  GEODE_FUNCTION(super_action_test)
  GEODE_FUNCTION_2(superstandardize,superstandardize_py)
  GEODE_FUNCTION(meaningless)
}
