...
static void popcounts_over_stabilizers_test(int steps) {
  ...
  for (int step=0;step<steps;step++) {
    ...
    GEODE_ASSERT(fast.x+fast.y==fast.z);
    // Count the slow way 
...
void wrap_count() {
  GEODE_FUNCTION(count_boards)
  GEODE_FUNCTION(estimate_supercount_boards)
  GEODE_FUNCTION(popcounts_over_stabilizers_test)
  GEODE_FUNCTION(sum_section_counts)
}
