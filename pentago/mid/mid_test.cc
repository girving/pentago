  ...
  for (const int i : range(results.size())) {
    ...
    for (const int a : range(2)) {
      ...
      for (const int s : range(256))
        if (known[s])
          PENTAGO_ASSERT_EQ(correct[s], value(rs, s) >= a);
  ...
  for (const int slice : range(30, 35+1)) {
    for (int i = 0; i < 8192; i++) {
      ...
      for (const bool middle : {false, true}) {
        ...
        PENTAGO_ASSERT_EQ(values.size(), boards.size());
...
TEST(mid, subsets) {
  ...
  for (const int n : range(18+1)) {
    for (const int k : range(4)) {
      ...
      switch (k) {
        case 0:
          PENTAGO_ASSERT_EQ(get(sets, a++), 0);
          break;
        case 1:
          for (const int i : range(n))
            PENTAGO_ASSERT_EQ(get(sets, a++), i);
          break;
        case 2:
          for (const int i0 : range(n))
            for (const int i1 : range(n))
              if (i0 > i1)
                PENTAGO_ASSERT_EQ(get(sets, a++), i1|i0<<5);
          break;
        case 3:
          for (const int i0 : range(n))
            for (const int i1 : range(n))
              for (const int i2 : range(n))
                if (i0 > i1 && i1 > i2)
                  PENTAGO_ASSERT_EQ(get(sets, a++), i2|i1<<5|i0<<10);
          break;
      }
