...
binary(bool b0, bool b1, bool b2, bool b3, bool b4, bool b5, bool b6, bool b7) {
...
// TODO(girving): Convert filter-test to C++ and move this there
static Array<Vector<int,3>> __attribute__((unused))
count_outer_causal_cases(Array<const Vector<super_t,2>,4> data) {
...
// TODO(girving): Convert filter-test to C++ and move this there
static Array<Vector<int,3>> __attribute__((unused))
count_causal_cases(Array<const Vector<super_t,2>,4> data) {
...
outer_causal_filter(Array<const Vector<int,3>> table, Array<Vector<super_t,2>,4> data) {
  GEODE_ASSERT(table.size()==81);
  const auto shape = data.shape();
...
// TODO(girving): Convert filter-test to C++ and move this there
static void __attribute__((unused))
inverse_outer_causal_filter(Array<const Vector<int,3>> table, Array<Vector<super_t,2>,4> data) {
  GEODE_ASSERT(table.size()==81);
  const auto shape = data.shape();
  for (int i0=1;i0<shape[0];i0++)
  for (int i1=1;i1<shape[1];i1++)
...
// TODO(girving): Convert filter-test to C++ and move this there
static void __attribute__((unused))
arbitrary_causal_filter(Array<const Vector<int,3>> table, Array<Vector<super_t,2>,4> data) {
  GEODE_ASSERT(table.size()==6561);
  const auto shape = data.shape();
  for (int i0=shape[0]-1;i0>0;i0--)
  for (int i1=shape[1]-1;i1>0;i1--)
  for (int i2=shape[2]-1;i2>0;i2--)
...
// TODO(girving): Convert filter-test to C++ and move this there
static void __attribute__((unused))
inverse_arbitrary_causal_filter(Array<const Vector<int,3>> table, Array<Vector<super_t,2>,4> data) {
  GEODE_ASSERT(table.size()==6561);
  const auto shape = data.shape();
  for (int i0=1;i0<shape[0];i0++)
  for (int i1=1;i1<shape[1];i1++)
...
// TODO(girving): Convert filter-test to C++ and move this there
static Array<int,2> __attribute__((unused))
count_rotation_cases(Array<const Vector<super_t,2>,4> data) {
...
static inline void compact_chunk(uint8_t dst[256], const Vector<super_t,2> src[5]) {
  const super_t s[2][5] = {{src[0][0],src[1][0],src[2][0],src[3][0],src[4][0]},
                           {src[0][1],src[1][1],src[2][1],src[3][1],src[4][1]}};
...
Array<Vector<super_t,2>> uncompact(Array<const uint8_t> src) {
  ...
  GEODE_ASSERT(src.size()==(256*dst.size()+4)/5);
...
template<bool invert> static void wavelet_helper(RawArray<F3,4> data) {
  const auto shape = data.shape();
  const auto strides = pentago::strides(shape);
