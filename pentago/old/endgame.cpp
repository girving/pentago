...
struct read_helper_t : public boost::noncopyable {
  ...
  {
    GEODE_ASSERT(standard.x==reader.header.section);
    for (int a=0;a<4;a++)
      GEODE_ASSERT(data.shape[a]==(a<2?first_block_shape[reorder[a]]:reader.header.shape[reorder[a]]));
  }

  void process_block(Vector<uint8_t,4> block, RawArray<Vector<super_t,2>,4> block_data) {
    thread_time_t time(copy_kind,unevent);
    GEODE_ASSERT(block[reorder[0]]==i && block[reorder[1]]==j);
    const int k = block[reorder[2]], l = block[reorder[3]];
    GEODE_ASSERT(block_data.shape==reader.header.block_shape(block));
...
GEODE_UNUSED static board_t section_board(const section_t& section, const Vector<int,4>& I) {
...
struct sparse_sample_t : public Object {
  ...
  int block_index(const Vector<uint8_t,4>& block) const {
    GEODE_ASSERT(   (unsigned)block[0]<(unsigned)blocks[0]
...
GEODE_DEFINE_TYPE(sparse_sample_t)
...
struct write_helper_t : public boost::noncopyable {
  ...
  {
    GEODE_ASSERT(!first_pass || writer.header.section==first_pass->header.section);
    for (int a=0;a<4;a++)
      GEODE_ASSERT(data.shape[a]==(a<2?first_block_shape[order[a]]:writer.header.shape[order[a]]));
  }

  void process_block(Vector<uint8_t,4> block, Array<Vector<super_t,2>,4> first_pass_data) {
    thread_time_t time(copy_kind,unevent);
    GEODE_ASSERT(block[order[0]]==i && block[order[1]]==j);
    ...
    if (first_pass) {
      GEODE_ASSERT(first_pass_data.shape==block_shape);
...
template<bool turn,bool final> struct compute_helper_t {
  ...
  {
    for (int a=0;a<4;a++)
      GEODE_ASSERT((a==2 || dest.shape[a]==src2.shape[a]) && (a==3 || dest.shape[a]==src3.shape[a]));
    GEODE_ASSERT(section==section.standardize<8>().x); // Prevent accidental computation of unnecessary data
    GEODE_ASSERT(dest.shape[2]==rmin[2].size());
    GEODE_ASSERT(dest.shape[3]==rmin[3].size());
    GEODE_ASSERT(section.counts[order[2]].sum()==9 || src2.shape[2]==rotation_minimal_quadrants(section.counts[order[2]]+move).x.size());
    GEODE_ASSERT(section.counts[order[3]].sum()==9 || src3.shape[3]==rotation_minimal_quadrants(section.counts[order[3]]+move).x.size());
  }
...
void wrap_endgame() {
  GEODE_FUNCTION(endgame_read_block_slice)
  GEODE_FUNCTION(endgame_write_block_slice)
  GEODE_FUNCTION(endgame_compute_block_slice)
  GEODE_FUNCTION(endgame_verify)
  GEODE_FUNCTION(endgame_sparse_verify)

  typedef sparse_sample_t Self;
  Class<Self>("sparse_sample_t")
    .GEODE_INIT(const supertensor_writer_t&,int)
    .GEODE_GET(boards)
    .GEODE_GET(wins)
    ;
}
