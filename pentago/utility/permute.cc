...
static inline uint64_t fe2_encrypt(const int bits, const uint128_t key, const uint64_t x) {
  ...
  // Three rounds of FE2
  L = ma&(L+uint32_t(threefry(key,uint64_t(1)<<32|R))); // round 1: s = a
  R = mb&(R+uint32_t(threefry(key,uint64_t(2)<<32|L))); // round 2: s = b
  L = ma&(L+uint32_t(threefry(key,uint64_t(3)<<32|R))); // round 3: s = a
