...
template<bool aggressive,bool debug> static inline superdata_t GEODE_ALWAYS_INLINE super_shallow_evaluate(const int depth, const side_t side0, const side_t side1, const super_t wins0, const super_t wins1, const super_t interesting) {
...
vector<Tuple<rotated_board_t,score_t>> super_evaluate_children(const bool aggressive, const int depth, const board_t board, const Vector<int,4> rotation) {
  ...
  for (const side_t move : results.moves) {
    ...
    if (results.immediate_wins.get_default(move,super_t(0))(rotation))
      children.push_back(tuple(tuple(after,as_tuple(rotation)),to_score(aggressive,report_depth,true)));
    else {
      ...
      for (Vector<int,4> r : single_rotations)
        if (info.known(rotation+r)) {
          children.push_back(tuple(tuple(after,as_tuple(rotation+r)),to_score(aggressive,report_depth,!info.wins(rotation+r))));
        }
    }
  }
...
void wrap_superengine() {
  GEODE_FUNCTION(super_evaluate)
  GEODE_FUNCTION(super_evaluate_children)
  GEODE_FUNCTION(set_super_move_limit)
  GEODE_FUNCTION(set_super_debug)
  GEODE_FUNCTION(set_block_cache)
}
