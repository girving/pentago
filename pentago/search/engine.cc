...
#include "pentago/search/engine.h"
...
static inline score_t quick_evaluate(board_t board) {
...
static score_t evaluate_recurse(int depth, board_t board) {
...
template<bool black> static inline score_t simple_quick_evaluate(side_t side0, side_t side1) {
...
template<bool black> static score_t simple_evaluate_recurse(int depth, side_t side0, side_t side1) {
  STAT(total_expanded_nodes++);
  STAT(expanded_nodes[depth]++);
